...
#set key off
set output 'mpi_multirun_system_throughput_selectivity_min_max.eps'
set title "MPI System Throughput for Data Selectivity Workflow"
plot 'min-max-mpi.dat' using ($1):($3) with lines title "mean",\
     'min-max-mpi.dat' using ($1):($2) with lines title "min",\
     'min-max-mpi.dat' using ($1):($4) with lines title "max",\

set output 'wq-mpi_multirun_system_throughput_selectivity_min_max.eps'
set title "WQ-MPI System Throughput for Data Selectivity Workflow"
plot 'min-max-wq-mpi.dat' using ($1):($3) with lines title "mean",\
     'min-max-wq-mpi.dat' using ($1):($2) with lines title "min",\
     'min-max-wq-mpi.dat' using ($1):($4) with lines title "max",\
